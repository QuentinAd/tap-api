...
def do_sync(access_key, base, symbols, start_date):
    ...
    try:
        while datetime.strptime(next_date, DATE_FORMAT) <= datetime.utcnow():
            logger.info('Replicating exchange rate data from %s using base %s',
                        next_date,
                        base)
            response = request(base_url + next_date, {'access_key': access_key,'base': base, 'symbols': symbols})
    ...
    singer.write_state(state)
    #with open("tap-state.json", mode= "w") as file:
...
def main():
    ...
    parser.add_argument(
        '-s', '--state', help='State file', required=True)
    ...
    start_date = state.get('latest_date') or config.get('start_date') or datetime.utcnow()
    start_date = singer.utils.strptime_with_tz(start_date).date().strftime(DATE_FORMAT)

    do_sync(access_key, base, symbols, start_date)

    if args.state:
        with open(args.state) as file:
            json_decoded = json.load(file)

        json_decoded['latest_date'] = do_sync.state

        with open(args.state, 'w') as file:
